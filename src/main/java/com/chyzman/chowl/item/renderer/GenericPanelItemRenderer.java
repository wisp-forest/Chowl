...
public class GenericPanelItemRenderer implements BuiltinItemRendererRegistry.DynamicItemRenderer {
    ...
    public void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        stacks.add(panel.displayedVariant(stack).toStack());
        ...
        if (renderScale < 1) {
            ...
            matrices.translate(-renderScale/2, -renderScale/2, 0);
        }
        matrices.translate(-renderScale, -renderScale, 0);
        matrices.scale((float) (renderScale), (float) (renderScale), 1);
        for (int i = 0; i < stacks.size(); i++) {
            matrices.push();
            matrices.translate(1- i % (1/renderScale), 1- (float) (int) (i / (1/renderScale)), 0);
            matrices.scale(4/3f, 4/3f, 1);
