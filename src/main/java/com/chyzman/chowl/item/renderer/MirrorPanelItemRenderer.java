...
public class MirrorPanelItemRenderer implements BuiltinItemRendererRegistry.DynamicItemRenderer {
    ...
    public void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        var baseModel = MinecraftClient.getInstance().getBakedModelManager().getModel(id("item/mirror_panel_base"));
        if (baseModel != null) {
        ...
        if (!(stack.getItem() instanceof MirrorPanelItem panel)) return;
        ...
        if (!displayStack.isEmpty()) {
            ...
            if (count.compareTo(BigInteger.ZERO) > 0 && !customization.hideCount()) {
                ...
                client.textRenderer.draw(panel.styleText(stack, Text.literal(amount)), -amountWidth / 2f + 0.5f, 0, Colors.WHITE, false, matrices.peek().getPositionMatrix(), vertexConsumers, TextRenderer.TextLayerType.NORMAL, 0, glowing ? LightmapTextureManager.MAX_LIGHT_COORDINATE : light);
                matrices.pop();
            }
        }
    }
